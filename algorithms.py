...
def generate_graph(original, params=None, planar=True):
#main entry point
    if params == None:
        params = {}
...
def edit_nodes_sequential_planar(G, node_edit_rate, node_growth_rate, tpl_data, params):
    ...
    for u in random.sample(G.nodes(), num_deleted_nodes):
        ...
        if minorizing_node_deletion: #connect the neighbors into a tree
            ...
            random.shuffle(nbrs)
            for nb_idx,nb in enumerate(nbrs[:-1]):
                new_edge = (str(nb), str(nbrs[nb_idx+1]))
                if not G.has_edge(*new_edge):
                    #assert new_edge[0] != new_edge[1]
                    G.add_edge(*new_edge)
                    if not planarity.is_planar(G):
...
def edit_edges_sequential_planar(G, edge_edit_rate, edge_growth_rate, tpl_data, params):
    ...
    added_edges = 0
    removed_edges = 0
    while added_edges < max(30, 4*target_edges_to_add):
        if num_remaining_edges_to_add <= 0:  #we might overshoot, hence <= 0 not ==
            break
        if npr.rand() > edge_welfare_fraction or len(deprived_nodes) == 0:
            head = random.choice(all_nodes)
...
def do_uncoarsen_planar(G_coarse, c_data, model_map, fine_model_map, params):
    ...
    #u or v must have been deleted
    c_data_edited = interpolate_edited_edges(G_coarse, c_data, model_map, fine_model_map, params)
    clean_c_data(G_coarse, c_data_edited)
    aggregates = c_data_edited['aggregates']
    trapped_edges = c_data_edited['trapped_edges']
    home_nodes = c_data_edited['home_nodes']
    merged_edges = c_data_edited['merged_edges']
    removed_edges = 0
    s1s2 = []
    non_planar_edges = []
    itr = 0
    ...
    for s1, s2 in G_coarse.edges_iter():
        ...
        for u, v in s1s2:
            ...
            #itr+=1
            #if len(added_edges)>=6 or itr >= len(G_coarse.edges()):
    ...
    added_edges = []
    itr = 0
    #random_edges = random.sample(non_planar_edges, len(non_planar_edges))
    #for u, v in non_planar_edges:
     #   G_fine.add_edge(u, v)
      #  if not planarity.is_planar(G_fine.edges()):
       #     G_fine.remove_edge(u, v)
        #    removed_edges += 1

    #print("length after", len(non_planar))

    #random_edges = random.sample(non_planar, len(non_planar))
